# Chord Diagram {#sec-chord-diagram}
...
```{r}
#| fig-cap: "加载 circlize 包并创建一个用于绘图的数据框"
...
```{r}
#| fig-cap: "初始化圆形图并添加第一个轨道, 轨道上绘制散点图"
...
```{r}
#| fig-cap: "使用循环和 layout 函数创建 9 个随机和弦图"
...
```{r}
#| fig-cap: "通过设置画布参数实现缩放效果, 并单独更新扇区 a 的样式以突出显示"
...
```{r}
#| fig-cap: "使用 `circlize` 包在圆形布局中添加不同类型的链接"
...
```{r}
#| fig-cap: 使用 circlize 包绘制的全球人口迁移弦图
...
```{r}
#| fig-cap: 使用 chorddiag 包绘制的头发颜色偏好交互式弦图
